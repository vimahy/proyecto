...
from account.forms import SettingsForm, DomicilioProfAccounForm,DomicilioFormSet
from account.forms import SocioForm
...
from .models import Division, Domicilio, Cuota, PublicacionDevuelta, DomicilioProfesional,PublicacionDevuelta
...
class ActualizarDomicilioProView(LoginRequiredMixin, FormView):


    template_name = "account/actualizar_profesional.html"
    form_class = DomicilioProfAccounForm
    messages = {
        "domicilio_profesional_actualizado": {
            "level": messages.SUCCESS,
            "text": _("Domicilio Profesional Actualizado.")
        },
    }

    def get_initial(self):
        try:
            initial = super(ActualizarDomicilioProView, self).get_initial()
            initial["codigo_institucion"] = self.request.user.account.domicilioprofesional.codigo_institucion
            initial["codigo_postal"] = self.request.user.account.domicilioprofesional.codigo_postal
            initial["institucion"] = self.request.user.account.domicilioprofesional.institucion
            initial["dependencia"] = self.request.user.account.domicilioprofesional.dependencia
            initial["calle_numero_apartado_postal"] = self.request.user.account.domicilioprofesional.calle_numero_apartado_postal
    ...
    def update_domicilioProfesional(self, form):
        fields = {}
        if "codigo_institucion" in form.cleaned_data:
            fields["codigo_institucion"] = form.cleaned_data["codigo_institucion"].upper()
        if "codigo_postal" in form.cleaned_data:
            fields["codigo_postal"] = form.cleaned_data["codigo_postal"]
        if "institucion" in form.cleaned_data:
            fields["institucion"] = form.cleaned_data["institucion"].upper()
        if "dependencia" in form.cleaned_data:
            fields["dependencia"] = form.cleaned_data["dependencia"].upper()
        if "calle_numero_apartado_postal" in form.cleaned_data:
            fields["calle_numero_apartado_postal"] = form.cleaned_data["calle_numero_apartado_postal"].upper()
        if "colonia" in form.cleaned_data:
            fields["colonia"] = form.cleaned_data["colonia"].upper()
        if "municipio_ciudad" in form.cleaned_data:
        ...
        if fields:
            account = self.request.user.account.domicilioprofesional
    ...
    def form_valid(self, form):
        self.update_domicilioProfesional(form)
        if self.messages.get("domicilio_profesional_actualizado"):
            messages.add_message(
                self.request,
                self.messages["domicilio_profesional_actualizado"]["level"],
                self.messages["domicilio_profesional_actualizado"]["text"]
            )
        return redirect(self.get_success_url())
    ...
    def get_success_url(self, fallback_url=None, **kwargs):
        if fallback_url is None:
            fallback_url = settings.ACCOUNT_DOMICILIO_PROFESIONAL_REDIRECT_URL
        kwargs.setdefault("redirect_field_name", self.get_redirect_field_name())
        return default_redirect(self.request, fallback_url, **kwargs)
...
class ActualizarDomicilioView(LoginRequiredMixin, FormView):

    template_name = "account/actualizar_domicilio.html"
    form_class = DomicilioFormSet
    messages = {
        "domicilio_actualizado": {
            "level": messages.SUCCESS,
            "text": _("Domicilio actualizado.")
        },
    }
    ...
    def get_initial(self):
        try:
            initial = super(ActualizarDomicilioView, self).get_initial()
            initial["codigo_postal"] = self.request.user.account.domicilio.codigo_postal
            initial["calle_numero_apartado_postal"] = self.request.user.account.domicilio.calle_numero_apartado_postal
            initial["colonia"] = self.request.user.account.domicilio.colonia
    ...
    def update_settings(self, form):
        self.update_domicilio(form)





    def update_domicilio(self, form):
        fields = {}
        if "codigo_postal" in form.cleaned_data:
            fields["codigo_postal"] = form.cleaned_data["codigo_postal"]
        if "calle_numero_apartado_postal" in form.cleaned_data:
        ...
        if "colonia" in form.cleaned_data:
            fields["colonia"] = form.cleaned_data["colonia"].upper()
        if "municipio_delegacion" in form.cleaned_data:
            fields["municipio_delegacion"] = form.cleaned_data["municipio_delegacion"].upper()
        if "ciudad" in form.cleaned_data:
        ...
        if "domicilio_profesional" in form.cleaned_data:
            fields["domicilio_profesional"] = form.cleaned_data["domicilio_profesional"].upper()
        if fields:
            account = self.request.user.account.domicilio
            for k, v in fields.items():
                setattr(account, k, v)
            account.save()
    ...
    def form_valid(self, form):
        self.update_domicilio(form)
        if self.messages.get("domicilio_actualizado"):
            messages.add_message(
                self.request,
                self.messages["domicilio_actualizado"]["level"],
                self.messages["domicilio_actualizado"]["text"]
            )
        return redirect(self.get_success_url())
    ...
    def get_success_url(self, fallback_url=None, **kwargs):
        if fallback_url is None:
            fallback_url = settings.ACCOUNT_DOMICILIO_REDIRECT_URL
        kwargs.setdefault("redirect_field_name", self.get_redirect_field_name())
        return default_redirect(self.request, fallback_url, **kwargs)
...
class SettingsView(LoginRequiredMixin, FormView):
    ...
    def get_initial(self):
        ...
        initial["nombre"] = self.request.user.first_name
        initial["apellido_materno"] = self.request.user.account.apellido_materno
        initial["apellido_paterno"] = self.request.user.last_name
        initial["fecha_nacimiento"] = self.request.user.account.fecha_nacimiento
        initial["sexo"] = self.request.user.account.sexo
        initial["fotografia"] = self.request.user.account.fotografia
        initial["grado_academico"] = self.request.user.account.grado_academico
        initial["institucion"] = self.request.user.account.institucion
    ...
    def update_settings(self, form):
        ...
        #if self.update_email(form):
        #se manda una notificacionde cambio de corrreo
            #email = EmailMessage('Cambio de correo',
            #'Usted ha cambiado su correo y nombre por'+ " "+user.email,
            #to=[user.email])
        self.update_account(form)
        #Aqui se asigna el nombre de usuario con el correo
    ...
    def update_account(self, form):
        ...
        if "nombre"in form.cleaned_data:
            fields["first_name"] = form.cleaned_data["nombre"].upper()
        if "email"in form.cleaned_data:
            fields["email"] = form.cleaned_data["email"]
        if "apellido_paterno"in form.cleaned_data:
            fields["last_name"] = form.cleaned_data["apellido_paterno"].upper()
        if "apellido_materno" in form.cleaned_data:
            fields["apellido_materno"] = form.cleaned_data["apellido_materno"].upper()
        if "fecha_nacimiento" in form.cleaned_data:
            fields["fecha_nacimiento"] = form.cleaned_data["fecha_nacimiento"]
        if "sexo" in form.cleaned_data:
            fields["sexo"] = form.cleaned_data["sexo"]
        if "fotografia" in form.cleaned_data:
            fields["fotografia"] = form.cleaned_data["fotografia"]
        if "grado_academico" in form.cleaned_data:
            fields["grado_academico"] = form.cleaned_data["grado_academico"]
        if "ocupacion" in form.cleaned_data:
            fields["ocupacion"] = form.cleaned_data["ocupacion"]
        if "institucion" in form.cleaned_data:
            fields["institucion"] = form.cleaned_data["institucion"].upper()
        if "telefono" in form.cleaned_data:
            fields["telefono"] = form.cleaned_data["telefono"].upper()
        if "division" in form.cleaned_data:
            fields["division"] = form.cleaned_data["division"]
        if "tipo_domicilio" in form.cleaned_data:
            fields["tipo_domicilio"] = form.cleaned_data["tipo_domicilio"]
        if "publicacion" in form.cleaned_data:
